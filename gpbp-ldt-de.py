...
def load_data(_storage_client):
    ...
    df_dev_plan = read_csv_from_gcs(
        _storage_client, BUCKET_NAME, "decision_engine/inputs/municipal_dev_plan.csv"
    )
    
    return df_indicatorlist, df_projects, gdf_score_geom, df_score, df_dev_plan

df_indicatorlist, df_projects, gdf_score_geom, df_score, df_dev_plan = load_data(storage_client)
    ...
    with st.form("login_form"):
        ...
        if submit:
            ...
            try:
                ...
                st.error("⚠️ Authentication system not configured. Please contact administrator.")
                
def render_logout_button():
    """Render logout button in sidebar or top of page."""
...
with scatterplot:
    ...
    if highlight_txt.strip():
        ...
        if len(matches) == 1:
            name = matches[0]
            district_name = slice_scatter[slice_scatter["ENGLISH_NAME"] == name]["NAME_1"].reset_index(drop=True).iloc[0]
            sel = slice_scatter[slice_scatter["NAME_1"] == district_name]
            fig = highlight_municipality_2d(fig, sel, x_score_name, y_score_name, 
                                                indicator_x, indicator_y, name, 
                                                x_is_score, y_is_score)
            st.success(f"✅ Highlighted Municipality: **{name}**, part of **{district_name}** District")
        elif len(matches) > 1:
    ...
    if highlight_txt.strip():
        ...
        if len(matches) == 1:
            name = matches[0]
            district_name = slice_3d[slice_3d["Municipality"] == name]["District"].reset_index(drop=True).iloc[0]
            sel = slice_3d[slice_3d["District"] == district_name]
            fig_3d = highlight_municipality_3d(fig_3d, sel, name)
            st.success(f"✅ Highlighted Municipality: **{name}**, part of **{district_name}** District")
        elif len(matches) > 1:
            st.info(f"Found multiple matches: {', '.join(matches[:5])}...")
        else:
...
with choropleth:
    ...
    if highlight_txt_choropleth.strip():
        matches = find_municipality_match(highlight_txt_choropleth, name_lookup)

        if len(matches) == 1:
            name = matches[0]
            district_name = slice_choropleth[slice_choropleth["ENGLISH_NAME"] == name]["NAME_1"].reset_index(drop=True).iloc[0]
            sel = slice_choropleth[slice_choropleth["NAME_1"] == district_name]
            
            st.success(f"✅ Highlighted: **{name}**, part of **{district_name}** District")
            # Display spatial metrics
            render_spatial_metrics(slice_choropleth, sel, district_name)
            
            fig = highlight_municipality_choropleth(fig, sel, name)
                ...
                if st.button(ui_text['project_recommendations_button'], use_container_width=True):
                    ...
                    with st.spinner(ui_text['status_background_research'].format(region=region)):
                        ...
                        if cached_en:
                            ...
                            regional_summary_en = get_background_research(openai_client, en_region, en_category, df_dev_plan)
